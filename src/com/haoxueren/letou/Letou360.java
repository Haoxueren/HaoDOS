...
public class Letou360 {
	...
	public void luckyDraw(String username, String password) throws Exception {
		OkHttpClient okHttpClient = new OkHttpClient.Builder().cookieJar(
				new CookieJar() {
		...
		FormBody body = new FormBody.Builder().add("j_username", username)
				.add("j_password", password).add("button", "立即登录").build();
		Request request = new Request.Builder().url(loginUrl).post(body)
				.build();
		...
		Request request2 = new Request.Builder().url(luckDrawUrlSpec)
				.addHeader("Cookie", loginCookie).build();
		...
		// 进入投资界面开始抽奖；
		FormBody formBody = new FormBody.Builder().add("form", "form")
		...
		Request request3 = new Request.Builder().url(luckDrawUrlSpec)
				.post(formBody).addHeader("Cookie", loginCookie).build();
