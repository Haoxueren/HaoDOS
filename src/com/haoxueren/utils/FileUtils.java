...
public class FileUtils
{
	...
	/**
	 * @method getFilesBySuffixs 这是一个获取指定目录下以指定后缀名文件集合的方法，支持多级目录；
	 * @param dir
	 *            想要获取的目录；
	 * @param suffixs
	 *            要指定的文件的后缀名（可变参数）； 如果传入一个空字符，即代表任意后缀均可；
	 * @return 包含指定后缀名的文件的ArrayList集合；
	 */
	// 这是用来储存指定目录下所有文件的集合；
	...
	/**
	 * @method getFileByName 输入文件名，打开指定目录下的包含该文件名的文件； 如果对应多个文件，则提供一个有序列表供用户选择打开； 建议将程序的快捷方式集中到指定的目录中，以提高程序的运行效率；
	 * @param dirPath
	 *            要操作的目录路径，本程序支持递归多级目录；
	 * @param fileName
	 *            想要获取的文件名称，可以通过键盘录入；
	 * @return 要查找的文件的集合； 如果集合为空，说明没有符合条件的文件；
	 */
	public static List<File> getFileByName(ArrayList<File> fileList, String dirPath, String fileName)
			throws IOException
	{
		// 封装要搜索的范围；
	...
	/**
	 * @method getDisksFiles 这是一个获取电脑上所有磁盘内文件的方法；
	 * @return 一个包含了该电脑上所有文件(含隐藏文件)的集合；
	 */
	...
	/**
	 * @method getFilesBySuffixs 这是一个获取电脑中所有指定后缀名文件的方法；
	 * @param suffixs
	 *            要获取的指定文件的后缀名（可变参数）；
	 * @return 符合指定的后缀名的条件的文件的ArrayList集合；
	 */
